...
class ResNet18CIFAR(ContinualLearningModelWrapper):
    def __init__(self, seed: int, num_classes=10, dropout: float = 0.0, hidden_layer: Optional[List[int]] = None, hidden_activation_name="swish", pretrain_path: Optional[str] = None):
        super(ResNet18CIFAR, self).__init__(seed)
        self.in_planes = 64
...
if __name__ == "__main__":
    pseudo_input = torch.zeros((1, 3, 32, 32))
    model = ResNet18CIFAR(num_classes=100, dropout=0.25, hidden_layer=[512, 256])
